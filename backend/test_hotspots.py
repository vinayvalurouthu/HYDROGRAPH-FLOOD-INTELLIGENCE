...
def run_endpoint_request(method, path, label):
    try:
...
# 1. Full hotspot list
code, data = run_endpoint_request("GET", "/api/v1/hotspots", "GET /api/v1/hotspots")
if data:
...
# 2. Filtered by risk
code, data = run_endpoint_request("GET", "/api/v1/hotspots?risk=CRITICAL", "GET /api/v1/hotspots?risk=CRITICAL")
if data:
    print(f"       -> {data['count']} CRITICAL hotspots")

# 3. Summary
code, data = run_endpoint_request("GET", "/api/v1/hotspots/summary", "GET /api/v1/hotspots/summary")
if data:
...
# 4. Heatmap
code, data = run_endpoint_request("GET", "/api/v1/hotspots/heatmap", "GET /api/v1/hotspots/heatmap")
if data:
...
# 5. Single detail
code, data = run_endpoint_request("GET", "/api/v1/hotspots/CR-07", "GET /api/v1/hotspots/CR-07")
if data:
...
# 6. 404 test
code, data = run_endpoint_request("GET", "/api/v1/hotspots/NONEXISTENT", "GET /api/v1/hotspots/NONEXISTENT (expect 404)")
